-- RLS policies for YRO app

alter table public.posts enable row level security;
alter table public.newsletter_subscribers enable row level security;
alter table public.user_roles enable row level security;

-- Helper: is_admin() checks user_roles (SECURITY DEFINER avoids RLS recursion)
...
-- POSTS
-- Public reads published; authenticated can read own drafts; admins read all
...
-- Admin can insert/update/delete (inline check so RLS doesn't depend on is_admin() for insert)
...
-- SUBSCRIBERS
-- User can manage only their row
...
-- USER ROLES
-- Users can read their own row (needed so app can check role; is_admin would be circular otherwise)
